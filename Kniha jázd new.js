...
// Verzia: 8.1 | Dátum: September 2025 | Autor: ASISTANTO
...
// ==============================================
// ✅ OPTIMALIZOVANÉ v8.1:
//    - Správna extrakcia GPS z JSGeolocation objektov
//    - Využitie location.lat a location.lng properties
//    - Robustné error handling pre GPS
// ==============================================
...
function getDefaultZdrzanie() {
    try {
        ...
        if (!defaultsLib) {
            utils.addError(currentEntry, "Knižnica " + CONFIG.defaultsLibrary + " nenájdená", "getDefaultZdrzanie");
            return 0;
        }
...
/**
 * Extrahuje GPS súradnice z poľa miesta - OPRAVENÁ VERZIA
 * Používa správny JSGeolocation objekt s properties lat a lng
 */
function extractGPSFromPlace(place) {
    ...
    try {
        // JSGeolocation objekt má properties lat a lng (NIE metódy!)
        lat = gpsLocation.lat;
        lon = gpsLocation.lng;
        ...
        utils.addError(currentEntry, "Chyba pri čítaní GPS objektu: " + error.toString(), "extractGPSFromPlace");
        return null;
    }
    ...
    try {
        ...
        for (var i = 0; i < posadka.length; i++) {
            ...
            try {
                ...
                var sadzbyLib = libByName(CONFIG.sadzbyLibrary);
                if (!sadzbyLib) {
                    utils.addError(currentEntry, "Knižnica sadzieb nenájdená", "calculateWageCosts");
                    continue;
                }
                ...
                if (sadzbyZamestnanca && sadzbyZamestnanca.length > 0) {
                    ...
                    if (najnovsiaSadzba) {
                        ...
                        utils.addDebug(currentEntry, "    💵 Hodinovka: " + hodinovka + " €/h (platná od " + utils.formatDate(najnovsiDatum, "DD.MM.YYYY") + ")");
                    }
                }
                
            } catch (error) {
                utils.addError(currentEntry, "Chyba pri získavaní sadzby: " + error.toString(), "calculateWageCosts");
            }
            
            // Ak nemáme hodinovku, skús použiť MementoBusiness helper
            if (!hodinovka || hodinovka <= 0) {
                var empDetails = utils.getEmployeeDetails(zamestnanec, datum);
                if (empDetails && empDetails.hourlyRate) {
                    hodinovka = empDetails.hourlyRate;
                    utils.addDebug(currentEntry, "    💵 Hodinovka z MementoBusiness: " + hodinovka + " €/h");
                } else {
