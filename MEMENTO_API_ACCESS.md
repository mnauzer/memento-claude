...
```
{project_root}/memento-claude/python-utilities/
```
...
api = MementoAPI(api_key="your_api_key_here")
...
1. **Root .env** (`{project_root}/memento-claude/.env`)
   - Pre nové skripty
   - Minimalistický
   - V .gitignore

2. **Config .env** (`{project_root}/memento-claude/config/.env`)
...
- **MementoConfig:** `{project_root}/memento-claude/core/MementoConfig7.js`
- **Python utilities:** `{project_root}/memento-claude/python-utilities/`
- **Environment config:** `{project_root}/memento-claude/config/.env`
